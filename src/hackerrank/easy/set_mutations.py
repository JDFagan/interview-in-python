...
"""
def process_set_mutations(a: {}, mutations: []):
    for m in mutations:
        operation = m[0]
        other_set = m[1]
        operation_method = getattr(a, operation)
        operation_method(other_set)
...
for _ in range(num_other_sets):
    ...
    other_set = {int(x) for x in input().split()}
    mutations.append((operation, other_set))

mutated_a = process_set_mutations(a=a, mutations=mutations)
total_sum = 0
for i in mutated_a:
    total_sum += i
print(total_sum)
